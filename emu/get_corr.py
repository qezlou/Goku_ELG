...
class Corr():
    ...
    def populate_halos(self, pig_dir, hod_model, seed=42, model_params={}):
        ...
        """
        halos = self.load_hlo_cat(pig_dir)
        hod = halos.populate(hod_model, seed=seed, **model_params)
        self.logger.info("total number of HOD galaxies = ", hod.csize)
        print(hod.columns)

        self.logger.debug(f"number of centrals = { hod.compute((hod['gal_type']==0).sum())}")
        self.logger.debug(f"number of satellites = {hod.compute((hod['gal_type']==1).sum())}")
    ...
    def get_corr(self, pig_dir, r_edges, seed=42, corr_mode='1d', hod_model=Zheng07Model):
        ...
        #corr_gal_real.run()
        hod = self.populate_halos(pig_dir, hod_model=hod_model, seed=seed)
        # in z-space
        LOS = [0, 0, 1]
    ...
    def iterate_over_pigs(self, base_dir,  z=2.5, savedir = '/work2/06536/qezlou/Goku/corr_funcs'):
        ...
        for i, pig_dir in enumerate(pigs['pig_dirs']):
            corr_gal_zspace = self.get_corr(pig_dir, r_edges, seed=42, hod_model=hod_model)
...
if __name__ == '__main__':
    ...
    corr.iterate_over_pigs(args.base_dir, z=args.z, savedir=args.savedir)
