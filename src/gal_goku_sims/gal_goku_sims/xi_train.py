...
class Corr():
    ...
    def _get_corr(self, pig_dir, mass_th, z=2.5):
        ...
        r_edges = np.logspace(np.log10(0.03), np.log10(0.3), 5)
        r_edges = np.append(r_edges, np.logspace(np.log10(0.3), np.log10(2), 15)[1:])
        r_edges = np.append(r_edges, np.logspace(np.log10(2), np.log10(30), 30)[1:])
        r_edges = np.append(r_edges, np.logspace(np.log10(30), np.log10(60), 30)[1:])
        self.logger.info(f'r_edges = {r_edges.size}')
